# -*- coding: utf-8 -*-
"""
Agente MPT e MTE — Ministério Público do Trabalho e Ministério do Trabalho (VERSÃO FINAL 2026)
"""
...
Publica conteúdo sobre: portarias, notícias do MPT, MTE, legislação trabalhista, inspeções, etc.
"""
...
def salvar_post(dados, fonte_nome):
    try:
        ...
        tags        = dados.get("tags", ["MPT", "Legislação"])
        tags_json   = json.dumps(tags, ensure_ascii=False)
        first_tag   = tags[0] if tags else "Legislação"
...
def main():
    print(f"\nAgente MPT/MTE — {HOJE.strftime('%d/%m/%Y')} [VERSÃO FINAL]")
    print("=" * 70)
    ...
    print(f"\nTotal publicado: {publicados} artigo(s) do MPT/MTE.")
